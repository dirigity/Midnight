...
(*) _DO NOT mess with DRM protected or copyrightable content. If you have legitimate access to it you will have probably agreed to a copyright contract, which you need to follow. Be aware of the terms and conditions of your apps_.

(**) _Apps will use secure ways to transfer important data. Using midnight the security of the conection will be mostly unharmed, but the data will get stored insecurely on your computer, wich is another way attackers can get ahold of it._ 
...
## 1
On the attacker's machine clone the repo, optionaly execute "npm install" to update dependencies, and run "node main.js" (`bash` and `openssl` must be installed and on the PATH). Midnight is compatible with windows if you are using WSL or cygwin, I recomend MobaXterm as a enviroment manager for cygwin.

## 2
Change the IP of the victim's DNS provider to use the IP of the attacker's machine. On linux modify the configuration with `sudo nano /etc/resolv.conf`. Otherwise look up a tutorial.
...
Congratulations, you are done! Try opening the url example.com on your browser and if everyting is OK you will see the attack in action.

# cheatsheet about stuff I've needed while developing
